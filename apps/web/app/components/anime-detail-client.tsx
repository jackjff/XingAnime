...
type Episode = { id: string; title: string; number: number | null; releaseDate: string | null; sources?: Array<{ source: string; id: string; slug: string }> };
...
function mergeSourceChoices(...groups: SourceChoice[][]): SourceChoice[] {
  const choices = new Map<string, SourceChoice>();
  groups.flat().forEach((choice) => choices.set(choice.source, choice));
...
export function AnimeDetailClient({ source, slug }: { source: string; slug: string }) {
  ...
  const [selectedSource, setSelectedSource] = useState<SourceChoice>({ source, slug });
  const [sourceChoices, setSourceChoices] = useState<SourceChoice[]>([]);
  const activeSources = sourceChoices.length ? sourceChoices : detail ? [{ source: detail.source, slug: detail.slug }] : [];
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await fetch(`${apiBaseUrl}/api/v1/sources/${encodeURIComponent(selectedSource.source)}/anime/${encodeURIComponent(selectedSource.slug)}?includeEpisodes=false`, { signal: controller.signal });
        ...
        setSourceChoices((current) => mergeSourceChoices(current, [selectedSource], payload.data.availableSources ?? []));
      } catch (cause) {
  ...
  useEffect(() => {
    ...
    setEpisodeError(null);
    fetch(`${apiBaseUrl}/api/v1/sources/${encodeURIComponent(selectedSource.source)}/anime/${encodeURIComponent(selectedSource.slug)}/episodes?${params.toString()}`, { signal: controller.signal })
      .then(async (response) => {
      ...
      .then((payload) => { if (!controller.signal.aborted) { setEpisodes(payload.data ?? []); setEpisodeMeta(payload.meta); } })
    ...
    return () => controller.abort();
  }, [detail, episodePage, episodeQuery, selectedSource]);
          ...
          <>
            <section className="detail-hero">
              ...
              <div className="detail-copy">
                <p className="eyebrow"><span className="live-dot" /> DETAIL ANIME · {selectedSource.source.toUpperCase()}</p>
                <h1>{detail.title}</h1>
                ...
                <div className="detail-actions">
                  {firstEpisode ? <a className="primary-button" href={`/watch/${selectedSource.source}/${firstEpisode.id}`}>▶ Mulai Episode 1</a> : <span className="primary-button disabled">Episode belum tersedia</span>}
                ...
                <div className="source-switcher" id="source-options">
                  ...
                  {activeSources.map((available) => { const provider = providers.find((candidate) => candidate.id === available.source); return provider ? <button type="button" aria-pressed={provider.id === selectedSource.source} className={provider.id === selectedSource.source ? 'source-chip selected' : 'source-chip'} onClick={() => setSelectedSource(available)} key={provider.id}>{provider.label}</button> : null; })}
                </div>
              </div>
            </section>
